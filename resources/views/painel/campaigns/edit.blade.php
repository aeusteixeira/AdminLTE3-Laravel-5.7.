...
@section('content')
    ...
    <form action="{{ route('mkt.campaigns.update', ['id' => $campaign->id]) }}" method="POST">
    @csrf
    @method('PUT')
        <div class="card-body">
                ...
                <section class="form-horizontal">
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                            <input required type="text" name="name_private" class="form-control" id="name_private" placeholder="Ex: Programa de Bolsas" value="{{  $campaign->name_private }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="description_private" class="col-sm-2 col-form-label">Descrição</label>
                            <div class="col-sm-10">
                            <textarea required name="description_private" class="form-control" id="description_private" placeholder="Pessoas inscritas nessa campanha são pessoas interessadas em bolsas de estudo, portanto usar o script feito para o Programa de Bolsas ao fazer contato...">{{ $campaign->description_private }}</textarea>
                            </div>
                        </div>
                    </div>
                </section>
                            ...
                            <div class="form-group">
                                <div class="form-group">
                                  ...
                                  <select class="form-control" name="status" id="status">
                                    ...
                                    <option value="finished" {{ $campaign->status == 'finished' ? 'selected' : ''}}>Finalizada</option>
                                  </select>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <label for="template_id">Template</label>
                                            <select class="form-control" name="template_id" id="template_id">
                                                @foreach ($templates as $template)
                                                    <option data-img="https://i.picsum.photos/id/970/900/900.jpg" value="{{ $template->id }}" {{ $campaign->template_id == $template->id ? 'selected' : ''}}>{{ $template->name }}</option>
                                                @endforeach
                                              </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="layout_id">Layout</label>
                                            <select class="form-control" name="layout_id" id="layout_id">
                                                @foreach ($layouts as $layout)
                                                    <option value="{{ $layout->id }}" {{ $campaign->layout_id == $layout->id ? 'selected' : ''}}>{{ $layout->name }}</option>
                                                @endforeach
                                            </select>
                                    </div>
                                </div>
                                ...
                                <select class="form-control" id="unit_id" name="unit_id">
                                      <option value="0">Todas as unidades</option>
                                        @foreach ($units as $unit)
                                            <option value="{{ $unit->id }}" {{ $campaign->unit_id == $unit->id ? 'selected' : ''}}>{{ $unit->name }}</option>
                                        @endforeach
                                  </select>
                        ...
                        <div class="card-body">
                            <div class="form-group row">
                                <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                                    <input type="checkbox" value="1" name="redirect" class="custom-control-input" id="redirect" {{ $campaign->redirect == 1 ? 'checked' : ''}}>
                                    <label class="custom-control-label" for="redirect" >Redirecionar usuário ao enviar dados?</label>
                                </div>
                            </div>
                            <div class="form-group row {{ $campaign->redirect == 1 ? '' : 'd-none'}}" id="for_redirect">
                    ...
                    <div class="card card-outline card-success">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <textarea rows="5" sentences required name="default_whatsapp_message" class="form-control" id="default_whatsapp_message" placeholder="Parabéns! Você se recentemente cadastrou no em nosso Programa de Bolsas. Estou te encaminhando essa mensagem informando que você foi comtemplado com uma bolsa de 60% em qualquer um de nossos cursos.">{{ $campaign->default_whatsapp_message }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <textarea rows="5" required name="default_email_message" class="form-control" id="default_email_message">{{ $campaign->default_email_message }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12 col-lg-4">
                            <div class="form-group">
                                ...
                                <input required type="text" name="name_public" class="form-control" id="name_public" placeholder="Ex: Bolsas de até 60%!" value="{{ $campaign->name_public }}">
                            </div>
                            <div class="form-group">
                                <label for="information_public">Descrição</label>
                                <textarea required rows="10" name="information_public" class="form-control" id="information_public" placeholder="Inscreva-se agora e adquira bolsas de até 60% de desconto em diversos cursos">{{ $campaign->information_public }}</textarea>
                            </div>
                        </div>
                        <div class="col-sm-12 col-lg-8">
                            <div class="card">
                                ...
                                <div class="card-body bg-primary">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-12 col-lg-10">
                                            <div class="card">
                                                <div class="card-body">
                                                    <h3 class="text-dark text-center" id="for_name_public">
                                                        {{ $campaign->name_public }}
                                                    </h3>
                                                    <p class="text-dark text-center"  id="for_information_public">
                                                        {{ $campaign->information_public }}
                                                    </p>
